...
}  //doesn't give exact value of the width, but it's pretty close
export const calculateH = (expectedH: number): number => ((expectedH + gridMargins[1]) / (gridRowHeight + gridMargins[1]))
export const getDimensions = (ref:RefObject<HTMLDivElement>):[number, number] => {
  if (ref.current) {
...
export const tasksInOverdue = (tasks: ITask[]): ITask[] => {
  return tasks.filter((task) =>
    task.deadline > localDateTimestampMS && !task.isCompleted)
}
export const tasksUpcoming = (tasks: ITask[]): ITask[] => {
  return tasks.filter((task) =>
    task.deadline < localDateTimestampMS && !task.isCompleted)
}
...
export const cutName = (name: string, maxLength: number) => {
